/**
 * FROSTBYTE #088 — MINT EDITION #4 & SEND TO FRIEND
 */
...
const { create, mplCore, transferV1 } = require('@metaplex-foundation/mpl-core');
...
const FRIEND_ADDRESS = "CUPxyEgEvw1Yi8n2C62SPVVYcMiMu4exMBrDz4eS8fXA";
...
// =========== MAIN ===========
async function main() {
    console.log(`❄️ **MINTING EDITION #${EDITION} FOR FRIEND** ❄️`);
    ...
    const jsonName = `Frostbyte_088_FP_${EDITION}_Friend.json`;
    ...
    try {
        ...
        // 2. TRANSFER
        console.log(`     Sending to ${FRIEND_ADDRESS}...`);
        await transferV1(umi, {
            asset: asset.publicKey,
            newOwner: publicKey(FRIEND_ADDRESS),
        }).sendAndConfirm(umi);
        ...
        const receipt = {
            ...
            signature: sig,
            owner: FRIEND_ADDRESS,
            metadataUri,
            timestamp: new Date().toISOString()
        };

        fs.writeFileSync(
            path.join(__dirname, `../data/mint-receipt-088-edition${EDITION}-friend.json`),
